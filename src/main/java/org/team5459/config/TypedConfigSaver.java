...
public final class TypedConfigSaver {
   ...
   *
   * <p>Intended for the committed config file (e.g. on a dashboard Save button press), where a
   * half-written or corrupted file would be read back at the next boot.
   *
   * @param jsonFile Destination file
   * @param document Configuration document to save
  ...
  public static void saveSafely(File jsonFile, ConfigDocument document) {
    ...
    try {
      ...
      try {
        ...
        // Some filesystems can't do an atomic rename; fall back to a plain move. Validation
        // already passed, so this is still safe, just not crash-proof against an interruption
